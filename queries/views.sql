DROP VIEW IF EXISTS dormitories;
DROP VIEW IF EXISTS rooms;
DROP VIEW IF EXISTS payments;
DROP VIEW IF EXISTS students;
DROP VIEW IF EXISTS issues;
DROP VIEW IF EXISTS administration_workers;
DROP VIEW IF EXISTS doorkeepers;
DROP VIEW IF EXISTS announcements;
...
CREATE VIEW rooms
AS
  select r.id, r.number, r.capacity, dorm.name as "dormitory", count(s.room_id) as "locators"
  from room r
    inner join dormitory dorm on dorm.id=r.dormitory_id
    inner join student s on s.room_id=r.id
  group by r.id, s.room_id, r.number, r.capacity, dorm.name;
...
CREATE VIEW issues
AS
  select i.date, i.content, p1.first_name as "student first name", p1.last_name as "student last name", r.number as "room number", p2.first_name as "doorkeeper first name", p2.last_name as "doorkeeper last name", dorm.name as "Dormitory"
